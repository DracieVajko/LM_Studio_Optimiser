...
class DatabaseManager:
    ...
    def _migration_008_best_columns(self, conn: sqlite3.Connection) -> None:
        """Persist run winner: best_config_id + pareto frontier ids."""
        for col, ddl in [
            ("best_config_id", "TEXT"),
            ("pareto_config_ids_json", "TEXT"),
        ]:
            try:
                conn.execute(f"ALTER TABLE runs ADD COLUMN {col} {ddl}")
            except Exception:
    ...
    def _migration_009_capability_snapshots(self, conn: sqlite3.Connection) -> None:
        """Per-run parameter registry snapshot (source + status per parameter)."""
        conn.execute("""
