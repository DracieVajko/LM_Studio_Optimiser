...
def _split_generation(config_data: dict) -> tuple[dict, dict | None]:
    """Split nested generation settings from stored config_json.

    Generation settings live nested inside the config_json column (no schema
    migration); everything else maps onto LoadConfiguration fields.
    """
...
class RunRepository:
    ...
    def save(self, run: OptimizationRun) -> str:
        ...
        best_config_id = str(run.best_config_id) if run.best_config_id else None
        pareto_json = json.dumps([str(i) for i in run.pareto_config_ids or []])
        # Resolve hardware_id
            ...
            if existing:
                conn.execute(
                    ...
                    UPDATE runs SET
                        ...
                        best_config_id=?, pareto_config_ids_json=?
                    WHERE id=?
                """,
                    (
                        run.model.id,
    ...
    def delete(self, run_id: str) -> bool:
        """Delete a run (configurations cascade). Returns True if removed."""
        with db_manager.get_connection() as conn:
            cursor = conn.execute("DELETE FROM runs WHERE id = ?", (run_id,))
            return cursor.rowcount > 0

    def delete_orphan_model(self, model_id: str) -> bool:
        """Delete a model row only when no runs reference it."""
        with db_manager.get_connection() as conn:
            left = conn.execute(
                "SELECT COUNT(*) FROM runs WHERE model_id = ?", (model_id,)
            ).fetchone()[0]
            if left:
                return False
    ...
    def _row_to_run(self, row: sqlite3.Row) -> OptimizationRun:
        ...
        try:
            ...
            benchmark_params = {}
        try:
            best_config_id = UUID(row["best_config_id"]) if row["best_config_id"] else None
        except Exception:
            best_config_id = None
        try:
            pareto_config_ids = (
                [UUID(i) for i in json.loads(row["pareto_config_ids_json"])]
                if row["pareto_config_ids_json"]
                else []
            )
        except Exception:
            pareto_config_ids = []
    ...
    def _row_to_config(self, row: sqlite3.Row) -> ConfigurationResult:
        ...
        if row["config_json"]:
            ...
            config_data, generation = _split_generation(config_data)
...
class ConfigurationRepository:
    ...
    def save(self, config: ConfigurationResult) -> str:
        ...
        if config.generation is not None:
            config_json = {**config_json, "generation": config.generation}
...
class CapabilityRepository:
    """Per-run parameter registry snapshots (source + status per parameter)."""

    def save(self, run_id: str | None, version: str, backend: str) -> int:
        """Record the current registry snapshot. Returns snapshot id."""
        from lm_optimizer.services.parameter_registry import snapshot

        matrix_json = json.dumps(snapshot())
        with db_manager.get_connection() as conn:
            cursor = conn.execute(
                """
                INSERT INTO capability_snapshots
                (run_id, lm_studio_version, backend, matrix_json)
                VALUES (?, ?, ?, ?)
            """,
                (str(run_id) if run_id else None, version, backend, matrix_json),
            )
            return cursor.lastrowid or 0

    def get_latest(self) -> dict | None:
        """Latest snapshot with parsed matrix."""
        with db_manager.get_connection() as conn:
            row = conn.execute(
                "SELECT * FROM capability_snapshots ORDER BY id DESC LIMIT 1"
            ).fetchone()
            if not row:
                return None
            return {
                ...
                "run_id": row["run_id"],
                "created_at": row["created_at"],
                "lm_studio_version": row["lm_studio_version"],
                "backend": row["backend"],
