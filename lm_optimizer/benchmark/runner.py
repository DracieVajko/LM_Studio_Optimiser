...
class BenchmarkMetrics:
    ...
    estimated_ttft_ms uses the server-measured time_to_first_token_seconds when
    the native chat stats provide it, falling back to total_time*0.1 otherwise.
    """
...
class BenchmarkRunner:
    ...
    def __init__(self, client: LMStudioClient, reasoning: str | None = "off"):
        self.client = client
    ...
    async def run_benchmark(
        ...
        load_time_ms = 0.0
        try:
            # Load model with configuration (inside try: failures still unload)
        ...
        logger.info(
            ...
            gen_tok_s=self.get_avg_generation_tok_s(result),
        )
    ...
    async def _run_single_case(self, model_id: str, case: BenchmarkCase) -> BenchmarkMetrics:
        ...
        try:
            ...
            # Extract metrics from the converted OpenAI-shaped response
            usage = response.usage
            choice = response.choices[0] if response.choices else None
            output_text = choice.message.content if choice else ""
            if (
                not output_text.strip()
                and (self.reasoning in (None, "off"))
            ...
            if not output_text.strip():
                # Degenerate generation: must not poison speed stats.
            ...
            # Real TTFT/tok/s from native stats when available (R11/R12)
    ...
    def _aggregate_results(
        ...
        for test_name, test_metrics in by_test.items():
            ...
            # Use median for robustness; quality reads the most complete sample (R33)
            ...
            agg = BenchmarkMetrics(
                ...
                output_text=best_text_run.output_text,  # Use most complete run for quality check
            )
