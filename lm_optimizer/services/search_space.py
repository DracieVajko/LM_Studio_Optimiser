...
class SearchSpace:
    ...
    physical_batch_sizes: list[int] = field(default_factory=list)
    parallels: list[int] = field(default_factory=list)
    context_checkpoints_options: list[int] = field(default_factory=list)
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "physical_batch_sizes": self.physical_batch_sizes,
            "parallels": self.parallels,
            "context_checkpoints_options": self.context_checkpoints_options,
        }
    ...
    def estimate_size(self) -> int:
        ...
        return (
            ...
            * max(1, len(self.physical_batch_sizes))
            * max(1, len(self.parallels))
            * max(1, len(self.context_checkpoints_options))
        )
...
class SearchSpaceGenerator:
    ...
    def generate(
        ...
        # Stage-4 load dimensions (server defaults when unsupported)
        space.physical_batch_sizes = self._generate_physical_batch_sizes(caps, advanced)
        space.parallels = self._generate_parallels(caps, advanced)
        space.context_checkpoints_options = self._generate_context_checkpoints(caps, advanced)
    ...
    def _generate_physical_batch_sizes(
        self, caps: LMStudioCapabilities, advanced: dict
    ) -> list[int]:
        """Stage-4 candidates around the server default (512)."""
        if not caps.supports_physical_batch_size:
            return [512]
        if advanced.get("physical_batches"):
            return sorted(set(advanced["physical_batches"]))
        return [256, 512, 1024]

    def _generate_parallels(self, caps: LMStudioCapabilities, advanced: dict) -> list[int]:
        """Stage-4 candidates around the server default (4), workload-aware."""
        if not caps.supports_parallel:
            return [4]
        if advanced.get("parallels"):
            return sorted(set(advanced["parallels"]))
    ...
    def _generate_context_checkpoints(
        self, caps: LMStudioCapabilities, advanced: dict
    ) -> list[int]:
        """Stage-4 candidates around the server default (32)."""
        if not caps.supports_context_checkpoints:
            return [32]
        if advanced.get("checkpoints"):
            return sorted(set(advanced["checkpoints"]))
        return [16, 32, 64]
