...
export const UI = {
    ...
    renderDashboard(status, modelsResponse) {
        ...
        const connected = !!(status.lm_studio && status.lm_studio.connected);
        ...
        return `
            <div class="space-y-8">
                ...
                <div class="card border-red-300 bg-red-50">
                    <div class="card-body">
                        <h3 class="font-semibold text-red-700">LM Studio DISCONNECTED</h3>
                        <p class="text-sm text-gray-600">LM Studio is not reachable.
                        Start LM Studio, enable the Developer server, then
                        <a class="underline" href="/settings">Open Settings</a>
                        to check the URL. Optimization, benchmarks and live
                        progress stay disabled until the connection works.</p>
                        <p class="text-xs text-gray-500 mt-1">Last error: ${(status && status.error) || 'connection failed or timed out'}</p>
                    </div>
                </div>`}
        ...
        return `
            <div class="space-y-4">
                ...
                <div class="p-3 bg-blue-50 rounded-lg border border-blue-100 space-y-1 text-sm">
                    ...
                    <div><span class="text-gray-500">Strategy</span><br>
                        Phase A speed context (frozen): ${advanced.max_context <= 4096 ? advanced.max_context : 2048}<br>
                        Phase A quality context (frozen): ${advanced.max_context}<br>
                        Speed: staged cheap probes (no full suite until finalists)<br>
                        Quality: full 5-test suite on finalists only, then bounded recovery<br>
                        Validation: adaptive (2–5 reps by stability)<br>
                        Phase B max-context: ${advanced.optimize_context ? 'ON (opt-in)' : 'OFF'}<br>
                    </div>
                    <div><span class="text-gray-500">Estimated probes</span><br><span class="font-mono font-medium">${est}</span></div>
                    <div><span class="text-gray-500">Speed repetitions</span> adaptive by model speed &nbsp; <span class="text-gray-500">Validation</span> 2–5</div>
                </div>
    ...
    renderAdvancedSettings() {
        return `
            <div class="space-y-6">
                ...
                <div class="form-checkbox">
                    <input type="checkbox" id="optimize-context" class="form-checkbox-input advanced-toggle">
                    <label for="optimize-context">Optimize Maximum Context (Phase B, optional — runs after the runtime winner is final, runtime settings stay frozen)</label>
                </div>
            </div>
        `;
    },
        ...
        return `
            <div class="space-y-3">
                ...
                <div class="pt-2 border-t border-gray-200">
                    <p class="text-sm text-gray-500">Click "Start Optimization" to begin. Estimated probes: ~16 cheap probes + ≤5 quality finalists.</p>
                </div>
            </div>
        `;
    },

    // Error & Toast (retry reloads the current view; never a dead end).
    renderError(message, retry = false) {
        return `
            <div class="card">
                <div class="card-body text-center py-12">
                    ...
                    ${retry ? '<button class="btn btn-outline mt-4" onclick="window.location.reload()">Retry</button>' : ''}
                </div>
            </div>
        `;
    },

    // Run-load failure card: HTTP status + reason + retry (never a spinner).
    renderRunError(runId, status, reason) {
        return `
            <div class="card">
                <div class="card-body text-center py-12">
                    <h3 class="mt-2 text-lg font-medium text-gray-900">Unable to load run details.</h3>
                    <p class="mt-1 text-sm text-gray-500 font-mono">Run: ${runId || '—'}</p>
                    <p class="mt-1 text-sm text-gray-500">HTTP status: ${status || '—'}</p>
                    <p class="mt-1 text-sm text-gray-500">Reason: ${reason || 'unknown'}</p>
